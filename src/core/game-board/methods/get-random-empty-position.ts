...
const generateRandomPosition = (
  ...
  x: from
    ? Math.max(
        0,
        Math.min(ctx.dimensions.width - 1, from.x + Math.floor(Math.random() * range! * 2 - range!))
      )
    : Math.floor(Math.random() * ctx.dimensions.width),
  y: from
    ? Math.max(
        0,
        Math.min(ctx.dimensions.depth - 1, from.y + Math.floor(Math.random() * range! * 2 - range!))
      )
...
const isValidPosition = (
  ...
  getUnit: (pos: Position) => IUnit | null
): boolean => {
